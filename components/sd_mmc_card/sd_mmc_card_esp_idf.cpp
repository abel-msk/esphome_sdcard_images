...
// #endif
            ...
            // slot_config.cd
            // slot_config.cd = static_cast<gpio_num_t>(10);
            // slot_config.wp
        ...
        size_t SdMmc::read_file(const char *path, uint8_t *buf, int promise_len)
        {
        ...
        {
            ESP_LOGD(TAG, "SdMmc::open_file: %s", path);
            ...
            ESP_LOGD(TAG, "SdMmc::get meme: %s", path);
            std::vector<uint8_t> res;
            size_t fileSize = this->file_size(path);
            res.resize(fileSize);

            ESP_LOGD(TAG, "SdMmc::Read file: %s", path);
            ...
            ESP_LOGD(TAG, "File read complete. %d", res.size());
